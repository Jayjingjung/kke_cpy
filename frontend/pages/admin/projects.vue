<template>
  <div>
    <AdminCrudTable title="Projects" :columns="columns" :items="items"
    ...
    <Teleport to="body">
      <div v-if="showForm" class="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" @click.self="showForm = false">
        <div class="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
          <h2 class="text-xl font-bold mb-4">{{ editing ? 'Edit' : 'Add' }} Project</h2>
          <form @submit.prevent="handleSave" class="space-y-4">
            ...
            <div class="grid grid-cols-3 gap-4">
              ...
              <div><label class="block text-xs font-medium mb-1">Status</label>
                <select v-model="form.status" class="input"><option value="ongoing">Ongoing</option><option value="completed">Completed</option></select>
              </div>
              <div><label class="block text-xs font-medium mb-1">Location</label><input v-model="form.location" class="input"></div>
            </div>
            <div class="grid grid-cols-3 gap-4">
...
const form = reactive({ slug: '', title_lo: '', title_en: '', title_zh: '', category: '', status: 'ongoing', description_lo: '', description_en: '', description_zh: '', location: '' });
const columns = [{ key: 'id', label: 'ID' }, { key: 'title_en', label: 'Title' }, { key: 'category', label: 'Category' }, { key: 'status', label: 'Status' }];
async function load() { auth.loadFromStorage(); try { items.value = await authGet('/admin/projects', auth.accessToken); } catch {} }
function openForm(item?: any) { editing.value = item?.id || null; if (item) Object.assign(form, item); else Object.assign(form, { slug: '', title_lo: '', title_en: '', title_zh: '', category: '', status: 'ongoing', description_lo: '', description_en: '', description_zh: '', location: '' }); showForm.value = true; }
async function handleSave() { if (editing.value) await authPut(`/admin/projects/${editing.value}`, { ...form }, auth.accessToken); else await authPost('/admin/projects', { ...form }, auth.accessToken); showForm.value = false; await load(); }
async function handleDelete(id: number) { if (confirm('Delete?')) { await authDelete(`/admin/projects/${id}`, auth.accessToken); await load(); } }
onMounted(load);
</script>
